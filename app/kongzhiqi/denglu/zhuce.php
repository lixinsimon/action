...
if($cao=='moren' && $_W['ispost']){	
   ...
   $duanxin = ZiFuChuan_Zhuan_ShuZu($peizhi['duanxin']);	
	...
	$zhuce['danyuan']=$_W['danyuan'];
  $zhuce['zhanghao']=$zhanghao;		
	$zhuce['nicheng']=NiCheng();	
  $zhuce['shouji']=$zhanghao;		
  $zhuce['mima']=md5($_J['mima']);
	$zhuce['shijian']=SHIJIAN;
	$zhuce['ip']=GetIp();
	...
	$shu=array('zhanghao'=>$zhuce['zhanghao'],'mima'=>$_J['mima']);
...
// 	$is_zhanghao=Qu('he_huiyuan',array('danyuan'=>$_W['danyuan'],'zhanghao'=>$_J['shouji']));
// 	if($is_zhanghao){
// 		 json('手机号已注册',0);
// 	}
	...
	json('发送失败',0);
}elseif($cao=='tuijianma'){	
	$tuijianma=Cha('select hyid from '.BM('he_huancun').' where ip="'.GetIP().'" order by SHIJIAN DESC');
	json($tuijianma['hyid']);
}
mb('zhuce');
...
function NiCheng(){
	...
	$tou_num=rand(0,331);
	$wei_num=rand(0,325);
	return $nicheng=$nicheng_tou[$tou_num].$nicheng_wei[$wei_num];
}
?>