...
if($_W['shanghu']['shenfen']>3 || empty($_W['shanghu']['shenfen'])){
		XiaoXi('你的权限不够');
}
...
if($cao=='moren'){	
    $where='';
	if($_W['shanghu']['shenfen']>0 && $_J['d']>0){		
	    ...
	    $where=' where id in('.rtrim($ids,',').') and shenfen>='.$_W['shanghu']['shenfen'];
	}	
	...
	XiaoXi('添加成功',US('xitong/guanliyuan'));
}elseif($cao=='guanliyuan_bianji'){		
	if(empty($_J['id'])){
		XiaoXi('修改对象错误');
	}	
	if($_W['shanghu']['shenfen']>2 || empty($_W['shanghu']['shenfen'])){
		XiaoXi('你的权限不够');
	}
	$shu=array(    
	     'jieshushijian'=>strtotime($_J['jieshushijian']),	   
	...
	if($_W['ispost']){
		...
		if(!empty($_J['id'])){
		    ...
		    XiaoXi('更新管理组成功！',US('xitong/guanliyuan',array('b'=>'zulie')));	
		}else{
			ChaRu('he_guanliyuan_zu',$zu_);	
			XiaoXi('添加管理组成功！',US('xitong/guanliyuan',array('b'=>'zulie')));		
		}
	...
	XiaoXi('删除成功！',US('xitong/guanliyuan',array('b'=>'zulie')));
}
mb('guanliyuan');
?>