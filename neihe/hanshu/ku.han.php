...
function Cha($sql, $params = array()){
	return pdo()->fetch($sql, $params);
}
function ChaQuan($sql, $params = array(), $keyfield = '') {
	return pdo()->fetchall($sql, $params, $keyfield);
}
function ChaZongShu($sql, $params = array(), $column = 0) {
	$d=pdo()->fetchcolumn($sql, $params, $column);	
	return $d?$d:0;
}
function Gai($table, $data = array(), $params = array(), $glue = 'AND') {
	return pdo()->update($table, $data, $params, $glue);
}
function ChaRu($table, $data = array(), $replace = FALSE) {
	return pdo()->insert($table, $data, $replace);
}
function ShanChu($table, $params = array(), $glue = 'AND') {
	return pdo()->delete($table, $params, $glue);
}
function ChaRuID() {
	return pdo()->insertid();
}
function Qu($tablename, $condition = array(), $fields = array()) {
	return pdo()->get($tablename, $condition, $fields);
}

function YunSQL($sql, $params = array()) {
