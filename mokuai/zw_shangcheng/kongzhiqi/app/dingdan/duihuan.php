...
//      'zongjia' => $gwl['zongjia']-$gwl['jifendikou']-$quan_e+$gwl['yunfei'],
   ...
   if($dingdanhao && $dingdan_jichu['leixing']==2 ){
    	$p=Cha('select * from '.BM('zw_shangcheng_dingdan_shangpin').' where dingdanhao="'.$dingdanhao.'"');
    	if($p){
    		$TiXing['jin_e']='';
    		if($p['liquan']>0){
    			$TiXing['jin_e'].='礼券:'.$p['liquan'];
    		}
    		if($p['xunzhang']>0){
    			$TiXing['jin_e'].='勋章:'.$p['xunzhang'];
    		}        		
    	}
   }
