...
if($cao=='moren'){
	...
	if($_J['id']>0){		
	    ...
	    XiaoXi('修改成功',UHK('hexiao/index'));exit;
	}
	$l['ming']=trim($_J['nicheng']);
	$l['shijian']=SHIJIAN;					
	ChaRu('he_hexiao',$l);		
	XiaoXi('添加成功',UHK('hexiao/index'));
}elseif($cao=='shanchu'){
	...
	if($_J['guanjianci']){
		...
		$tiaojian.=' and (nicheng like "%'.$yonghu.'%" or xingming like "%'.$yonghu.'%" or zhanghao like "%'.$yonghu.'%")';
	}
	...
	CaoZuoJiLu('核销添加客服 '); 
    XiaoXi('添加成功',UH('hexiao',array('c'=>'kefu','hexiao_id'=>$_J['id'])));exit;	
	...
	ShanChu('he_hexiaoyuan',array('hyid'=>$_J['hyid'],'danyuan'=>$_W['danyuan']));
		CaoZuoJiLu('核销删除客服 '); 
    XiaoXi('删除成功',UH('hexiao',array('c'=>'kefu','hexiao_id'=>$_J['hexiao_id'])));exit;
}elseif($cao=='zhuangtai' &&  $_W['ispost'] ){
	if(empty($_J['zhuangtai']) && empty($_J['id']) ){
		XiaoXi('缺少参数');
	}
	CaoZuoJiLu('核销禁止启动'); 
	Gai('he_hexiao',array('zhuangtai'=>$_J['zhuangtai']),array('id'=>$_J['id']));
	json('成功');
}
