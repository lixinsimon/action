...
if(file_exists(IA_ROOT . '/gongyong/install.lock') && $action != 'finish') {
...
if($action == 'env') {
	...
	if(version_compare(PHP_VERSION, '5.3.0') == -1) {
		...
		$ret['php']['version']['remark'] = 'PHP版本必须为 5.3.0 或以上</a>';
	}
	...
	if($ret['php']['pdo']['ok']) {
		...
		if(!$ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['remark'] = '您的PHP环境虽然不支持 mysql_connect, 但已经支持了PDO, 这样系统是可以正常高效运行的, 不需要额外处理</a>';
		}
	} else {
		if($ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['pdo']['class'] = 'warning';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 但支持 mysql_connect, 这样系统虽然可以运行, 但还是建议你开启PDO以提升程序性能和系统稳定性</a>';
		} else {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['pdo']['class'] = 'danger';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 也不支持 mysql_connect, 系统无法正常运行</a>';
	...
	if($ret['php']['curl']['ok']) {
		...
		if(!$ret['php']['fopen']['ok']) {
			$ret['php']['curl']['remark'] = '您的PHP环境虽然不支持 allow_url_fopen, 但已经支持了cURL, 这样系统是可以正常高效运行的, 不需要额外处理</a>';
		}
	} else {
		if($ret['php']['fopen']['ok']) {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['curl']['class'] = 'warning';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 但支持 allow_url_fopen, 这样系统虽然可以运行, 但还是建议你开启cURL以提升程序性能和系统稳定性</a>';
		} else {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['curl']['class'] = 'danger';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 也不支持 allow_url_fopen, 系统无法正常运行</a>';
	...
	if($ret['php']['ssl']['ok']) {
		...
		$ret['php']['ssl']['remark'] = '没有启用OpenSSL, 将无法访问公众平台的接口, 系统无法正常运行</a>';
	}
	...
	if($ret['php']['gd']['ok']) {
		...
		$ret['php']['gd']['remark'] = '没有启用GD, 将无法正常上传和压缩图片, 系统无法正常运行</a>';
	}
	...
	if($ret['php']['dom']['ok']) {
		...
		$ret['php']['dom']['remark'] = '没有启用DOMDocument, 将无法正常安装使用模块, 系统无法正常运行</a>';
	}
	...
	if($ret['write']['root']['ok']) {
		...
		$ret['write']['root']['remark'] = '本地目录无法写入, 将无法使用自动更新功能, 系统无法正常运行</a>';
	}
	$ret['write']['data']['ok'] = local_writeable(IA_ROOT . '/gongyong');
	if($ret['write']['data']['ok']) {
		...
		$ret['write']['data']['remark'] = 'gongyong目录无法写入, 将无法写入配置文件, 系统无法正常安装. ';
	}
...
if($action == 'db') {
	if($ispost) {
		...
		if(empty($error)) {
			...
			$verfile = IA_ROOT . '/neihe/banben.php';
			$dbfile = IA_ROOT . '/gongyong/db.txt';
			...
			if($_POST['type'] == 'remote') {
				...
				$family ='v';
				//写入临时目录
				$tmpfile = IA_ROOT . '/zhiwi.tmp';
				file_put_contents($tmpfile, $archive);
				local_mkdirs(IA_ROOT . '/gongyong');
				file_put_contents(IA_ROOT . '/gongyong/config.php', base64_decode($ins['schemas']));
 ...
 * 版本号
 *
 * [weixin System] Copyright (c) 2016 ZHIWI.COM
 */
...
VER;
			...
			if(file_exists(IA_ROOT . '/index.php') && is_dir(IA_ROOT . '/app') && is_dir(IA_ROOT . '/houtai') && file_exists($verfile) && file_exists($dbfile)) {	

				$update_array=file_get_contents($dbfile);
				$update_array=ZiFuChuan_Zhuan_ShuZu($update_array);              
				foreach($update_array as $key=>$u){	
					$u['createTable'] = str_replace("\r", "\n", str_replace('ims_', $db['prefix'], $u['createTable']));

					mysql_query($u['createTable']);
				}
			} else {
				die('<script type="text/javascript">alert("你正在使用本地安装, 但未下载完整安装包, 请从微信官网下载完整安装包后重试.");history.back();</script>');
			}

			$salt = local_salt(8);
			//$password = sha1("{$user['password']}-{$salt}-{$authkey}");			
			$password=md5($user['password']);
			mysql_query("INSERT INTO {$db['prefix']}he_guanliyuan (yonghuming, mima,  zhuceshijian) VALUES('{$user['username']}', '{$password}','" . time() . "')");
			local_mkdirs(IA_ROOT . '/gongyong');
			file_put_contents(IA_ROOT . '/gongyong/config.php', $config);
			touch(IA_ROOT . '/gongyong/install.lock');
...
\$config['upload']['attachdir'] = 'gongyong/shangchuan';
...
function remote_install() {
	...
	$pars['version'] = '0.6';
	$pars['release'] = '';
	$url = 'http://addons.ZHIWI.COM/gateway.php';
	$ch = curl_init($url);
...
<html lang="zh-cn">
	<head>
		...
		<title>安装系统 - 微信 - 公众平台自助开源引擎</title>
	...
	echo <<<EOF
		<div class="panel panel-default">
			...
			<div class="panel-body" style="overflow-y:scroll;max-height:400px;line-height:20px;">
				<h3>版权所有 (c)2014-2015，商城团队保留所有权利。 </h3>
				<p>
					感谢您选择商城 - 微信公众平台自助开源引擎（以下简称weixin，weixin基于 PHP + MySQL的技术开发，全部源码开放。 <br />
					为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：
				</p>
				<p>
					<strong>一、本授权协议适用且仅适用于微信系统(weixin. 以下简称微信)任何版本，微信官方对本授权协议的最终解释权。</strong>
				</p>
				<p>
					<strong>二、协议许可的权利 </strong>
					<ol>
						<li>您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用。</li>
						<li>您可以在协议规定的约束和限制范围内修改微信源代码或界面风格以适应您的网站要求。</li>
				...
				<p>
					...
					<ol>
						...
						<li>电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始确认本协议并安装  weixin，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</li>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">						
						<label class="radio-inline">
							<input type="radio" name="type" value="local"{$insTypes['local']}{$disabled}> 安装商城系统
						</label>					
					</div>
				</div>
			</div>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input class="form-control" type="text" name="db[name]" value="weixin">
					</div>
				</div>
			</div>
		</div>
...
function tpl_install_finish() {	
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“商城开源引擎”系统，您现在可以: <a target="_blank" class="btn btn-success" href="./index.php">访问网站首页</a>
		<br />
	</div>
