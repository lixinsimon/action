<template>
	<view> 
		<view class="zongjine">
			...
			<navigator class="lijitixian" url="wode_tixian">立即提现</navigator>
		</view>
	...
	export default {
		...
		methods:{
			CSH:function(){
				var that = this;
				that.mieyou=0;							
				$z.post('m=zw_shangcheng&k=wode&x=qianbao',{ye:1,jitiao:that.ye*that.jitiao,zhuangtai:that.zhuangtai},function(shu){				  
					 if(shu.shi==1){							
						 that.shu=shu.shu;					 
					 }else{
						 that.mieyou=1;
			...
			jiazai:function(){
				var that=this;
				$z.post('m=zw_shangcheng&k=wode&x=qianbao',{ye:that.ye+1,jitiao:that.jitiao,zhuangtai:that.zhuangtai},function(shu){
					if(shu.shi==1){					
