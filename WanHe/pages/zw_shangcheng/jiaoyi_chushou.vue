	...
	<view>
		...
		<view class="body">
			...
			<view class="body_shurukuang">
				...
				<input type="text" v-model:value="shuliang"  :placeholder="'最多出售'+shu+'和券'"/>
			</view>
		</view>
		...
		<view class="body">
			<view class="body_biaoti">收款账号</view>
			<view class="body_shurukuang">
				<image src="https://wanhe.zhiwi.cn/gongyong/shangchuan/images/12/2018_12/sAdorJpaaJjzr4alRVYvxavrDyrro7.png" mode="widthFix"></image>
				<input type="text" v-model:value="shoukuan" placeholder="xsdkajfl@qq.com"/>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			CSH:function(){
				...
				this.shuliang='';
				this.jiage='';
				$z.post('m=zw_shangcheng&k=jiaoyi',{c:'jifen'},function(s){				
					that.shu=s.shu;   
				})
			},
			tijiao:function(){
				var that=this;
				$z.post('m=zw_shangcheng&k=jiaoyi',{shuliang:that.shuliang,jiage:that.jiage,shoukuan:that.shoukuan,c:'chushou'},function(shu){
					$z.toast(shu.shu);
					if(shu.shi){
						setTimeout(function(){
							uni.navigateTo({
