<template>
	<view> 
		...
		<view class="jinemingxi">
			<view class="jinemingxi_zi">明细</view>
			
			<view class="item" v-for="l in shu.lie">
				<view class="item_tou">
					<view class="fangshi">赠送和券</view>
				...
				<view class="item_wei">
					...
					<view class="jia_jian">{{l.songjifen}}和券</view>
				</view>
			</view>
	...
	export default {
		...
		methods:{
			CSH:function(){
				var that = this;
				that.mieyou=0;							
				$z.post('m=zw_shangcheng&k=dianpu&c=jifen',{ye:1,jitiao:that.ye*that.jitiao,zhuangtai:that.zhuangtai},function(shu){				  
					 that.shu=shu.shu;						 
				});
			},
			jiazai:function(){
				var that=this;
				$z.post('m=zw_shangcheng&k=dianpu&c=jifen',{ye:that.ye+1,jitiao:that.jitiao,zhuangtai:that.zhuangtai},function(shu){
					if(shu.shi){					
