...
(function($, window) {
	var rgbaRegex = /^rgb[a]?\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3}).*\)$/;
	var getColor = function(colorStr) {
		var matches = colorStr.match(rgbaRegex);
	...
	var TransparentFont = function(element, options) {
		this.element = element;
		this.options = $.extend({
			top: 0,
			offset: 150,
		...
		this._style = this.element.style;
		this._bgColor = this._style.color;
		var color = getColor(mui.getStyles(this.element, 'color'));
		if (color.length) {
			this._R = color[0];
			this._G = color[1];
			this._B = color[2];
			...
			throw new Error("元素背景颜色必须为RGB或RGBA");
		}
	};

	TransparentFont.prototype.initEvent = function() {
		window.addEventListener('scroll', this._bufferFn);
		window.addEventListener($.EVENT_MOVE, this._bufferFn);
	};
	TransparentFont.prototype.handleScroll = function() {
		var b = 1 - (window.scrollY - this.options.top) / this.options.offset;
		this._style.color = 'rgba(' + this._R * b + ',' + this._G * b + ',' + this._B *b + ',' +'1)';
	};
	TransparentFont.prototype.destory = function() {
		window.removeEventListener('scroll', this._bufferFn);
		window.removeEventListener($.EVENT_MOVE, this._bufferFn);
		this.element.style.color = this._bgColor;
		this.element.mui_plugin_transparent = null;
	};
	$.fn.transparentFont = function(options) {
		options = options || {};
		var transparentApis = [];
		this.each(function() {
			var transparentApi = this.mui_plugin_transparent;
			if (!transparentApi) {
				...
				transparentApi = this.mui_plugin_transparent = new TransparentFont(this, options);
			}
			transparentApis.push(transparentApi);
		});
		return transparentApis.length === 1 ? transparentApis[0] : transparentApis;
	...
	$.ready(function() {
		$('.mui-bar-transparent-font').transparentFont();
	});
})(mui, window);
